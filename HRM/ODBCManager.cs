...
namespace POS
{
    ...
    {     
        ...
        /// <summary>     
        /// Creates a new DSN entry with the specified values. If the DSN exists, 
        /// the values are updated.     /// </summary>     
        /// <param name="dsnName">Name of the DSN for use by client applications</param> 
        /// 
        /// <param name="description">Description of the DSN that 
        /// appears in the ODBC control panel applet</param>    
        /// /// <param name="server">Network name or IP address 
        /// of database server</param>    
        /// /// <param name="driverName">Name of the driver to use</param>    
        /// /// <param name="trustedConnection">True to use NT authentication, 
        /// false to require applications to supply username/password 
        /// in the connection string</param>     
        /// <param name="database">Name of the datbase to connect to</param>   
         public void CreateDSN(string dsnName, string description, string server, string driverName, bool trustedConnection, string database)     
         {
             //driverName = "SQ";
             // Lookup driver path from driver name
             var driverKey = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\ODBC\ODBCINST.INI\SQL Server");
             //var driverKey = Registry.LocalMachine.OpenSubKey(ODBC_INI_REG_PATH + driverName, false );
             if (driverKey == null) throw new Exception(string.Format("ODBC Registry key for driver '{0}' does not exist", driverName));
             string driverPath = driverKey.GetValue("Driver").ToString();
             // Add value to odbc data sources
             var datasourcesKey = Registry.LocalMachine.CreateSubKey(@"SOFTWARE\ODBC\ODBC.INI\ODBC Data Sources");
             //var datasourcesKey = Registry.LocalMachine.OpenSubKey(ODBC_INI_REG_PATH + "ODBC Data Sources");
             if (datasourcesKey == null) throw new Exception("ODBC Registry key for datasources does not exist");
             datasourcesKey.SetValue(dsnName, driverName);
             // Create new key in odbc.ini with dsn name and add values
             var dsnKey = Registry.LocalMachine.CreateSubKey("SOFTWARE\\ODBC\\ODBCINST.INI\\SQL Server");
